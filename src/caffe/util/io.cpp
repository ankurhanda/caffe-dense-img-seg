...
cv::Mat ReadBinary4File(const std::string &filename, const int height, const int width)
{
  std::ifstream ifb;
//  cv::Mat imdata(width,height,CV_32FC3);
  cv::Mat imdata(width,height, CV_32FC4);
  ...
  while (i<height && !ifb.eof())
  {
      ...
      float* row_ptr = imdata.ptr<float>(i);
...
//	  std::cout<<"value of j = "<< j << std::endl;
  ...
  //std::cout << "Total number of rows and columns = " << height <<", "<< width << std::endl;
...
//  cv::Mat cv_img_origin = ReadBinary4File(filename, height, width);
  if (!cv_img_origin.data) {
