...
void ImageSegDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  LOG(INFO)<<" Meta Data " << width <<", "<< height <<", "<< channels;
...
void ImageSegDataLayer<Dtype>::ShuffleImages() {
  ...
  shuffle(lines_.begin(), lines_.end(), std::default_random_engine(epoch_));

 // std::cout<<root_folder<<std::endl;
 
  //call_no = ( call_no + 1 ) % INT_MAX;
}
